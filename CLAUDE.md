...
**중기 (1~3개월) — Phase 2 데이터 고도화** (2026-02-27 상세화)

*1단계: 데이터 실연동 (1~2주)*
- [ ] 브이월드 실연동 — GET /api/land/parcel mock→실제 연속지적도 (키 발급완료, 만료 2026-08-26)
- [ ] KAMIS .env 설정 — KAMIS_API_ID/KEY 추가 → 실제 경매가 확인
- [ ] ASOS 과거 데이터 캐싱 — 10개 주산지 × 10년 일별 기상 → data/climate_cache/ JSON
- [ ] 중기예보 API 추가 — 3일 단기→10일 중기예보 확장 (같은 data.go.kr 키)

*2단계: 급지 v2 (2~3주)*
- [ ] DEM 지형 분석 — 해발고도+경사도 팩터 추가 (국토지리원 DEM 또는 SRTM)
- [ ] 일조 차단 분석 — 산지 차폐에 의한 일조 감소 반영
- [ ] 급지 변동 전망 — ASOS 과거 10년 기온 추세 → 5/10년 뒤 급지 예측
- [ ] 급지 UI 페이지 — 10개 주산지 급지 지도 + 팩터 레이더 차트

*3단계: 사용자 시스템 (2~3주)*
- [ ] PostgreSQL 설정 — DB 스키마, SQLAlchemy 모델 활성화 (Railway 또는 로컬)
- [ ] 사용자 인증 — 카카오 로그인 (NextAuth.js + 카카오 OAuth)
- [ ] 과수원 저장 — 설계/시뮬레이션 결과 저장·불러오기
- [ ] 알림 시스템 — 서리·폭우·가격급변 (카카오 알림톡 또는 웹 푸시)

*4단계: ML + 분석 고도화 (3~4주)*
- [ ] ML 작황 예측 — 규칙→통계→ML 단계적 (ASOS+KOSIS, scikit-learn)
- [ ] 병해충 예측 — 기온+습도+강수 조합 → 탄저병/갈반병 위험도
- [ ] Clarity 분석 → 60대 UX 개선 — 히트맵 기반 UI 리팩터

*5단계: 현장 요구 기능 (2~3주)*
- [ ] 인력 관리 — 시기별 인력 수요 캘린더, 인력사무소 연락처
- [ ] 정부 지원사업 정보 — 지역별 보조금/융자 리스트, 신청 기한 알림
- [ ] 비상연락망 — 인력사무소/농기센/재해보험 번호, 탭→전화

**장기 (3~12개월) — Phase 3-4 AI + 수익화**
- AI 병해충 진단, 3D 렌더링, 모바일 앱
- 음성 인터페이스 (STT+TTS) — 농작업 중 양손 사용 불가 → 음성 입출력 필수 (PJ01 TTS 자산 활용)
- 번역 (베트남·캄보디아어), 커뮤니티, 직거래 마켓
- B2B/B2G 계약, IoT/위성 연동, 급지 v3
...
### 다음 작업: 중기 로드맵 Phase 2 (1~3개월)

**1단계: 데이터 실연동 (1~2주)**
- [ ] 브이월드 실연동 — GET /api/land/parcel mock→실제 연속지적도 (키 발급완료, 만료 2026-08-26)
- [ ] KAMIS 실연동 확인 — https 수정 완료, 실제 경매가 응답 검증
- [ ] ASOS 과거 데이터 캐싱 — 10개 주산지 × 10년 일별 기상 → data/climate_cache/ JSON
- [ ] 중기예보 API 추가 — 3일 단기→10일 중기예보 확장 (같은 data.go.kr 키)
...
### 장기 로드맵 Phase 3-4 (3~12개월)
- [ ] AI 병해충 진단 (사진 기반)
- [ ] 3D 과수원 렌더링 (Three.js)
- [ ] 모바일 앱 (React Native) — 음성 인터페이스(STT+TTS) 필수 (농작업 중 양손 불가)
- [ ] 번역 (베트남·캄보디아어) — 외국인 노동자용 농작업 지시
- [ ] 커뮤니티 + 직거래 마켓
- [ ] B2B/B2G 계약 (농협, 지자체)
- [ ] IoT 센서 + 위성영상 연동
- [ ] 급지 v3 (위성 토양 분석)
